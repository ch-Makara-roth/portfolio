...
export async function GET(_req: NextRequest, { params }: { params: { slug: string } }) {
  const url = BASE ? `${BASE}/blog/posts/${params.slug}` : ''
  try {
    if (url) {
      ...
      const mapped = {
        ...
        data: {
          ...
          contentHtml: typeof data?.contentHtml === 'string' ? data.contentHtml : undefined,
        },
      }
    ...
    const mock = {
      ...
      data: {
        ...
        excerpt: 'Learn how to use React Hooks effectively in modern applications.',
        imageUrl: null,
        tags: ['React', 'Hooks'],
        content:
